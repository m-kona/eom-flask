...
def user_table():
    conn = sqlite3.connect('flasko.db')
    ...
    if request.method == "PUT":
        with sqlite3.connect('flasko.db') as conn:
            ...
            if incoming_data.get("product_name") is not None:
                ...
                with sqlite3.connect('flasko.db') as conn:
                    cursor = conn.cursor()
                    cursor.execute("UPDATE post SET product_name =? WHERE id=?", (put_data["product_name"], post_id))
                    conn.commit()
            ...
            if incoming_data.get("description") is not None:
                ...
                with sqlite3.connect('flasko.db') as conn:
                    cursor = conn.cursor()
                    cursor.execute("UPDATE product SET price =? WHERE id=?", (put_data["description"], post_id))
                    conn.commit()
